...
void Defense::lineTrace(){
  Vector v = {0,0};
  bool is_enough_time = (millis() - dash_start_time > dash_wait_time);
  if(line_angle == 400){
    state = lost_line;
  }
    ...
    //キーパーダッシュ
    if(isNeedTo_dash() != 0 && is_enough_time && on_dash == true){
      attack.startKeeperDash();  
      dash_start_time = millis(); // 開始時間を記録
    }
    else{
...
void Defense::selfTrace(){
  ...
  if(line_angle == 400){
    ...
    lost_count = 0;
    v = assembleV(lineV(),selfTraceV(),pwm_max);
    // if(line_angle == 500){
    //   v = TlineV(v);
    // }
    v = echo.backWallBlockV(v);
...
void Defense::lostLine(){
  ...
  //一定時間経ったら
  else if(lost_count > lost_count_max){
