...
void Defense::debugSerial(){
    ...
    //  Serial.print(",  time:");
    //  Serial.print(dash_interval_now);
      ...
      // Serial.print(echo.SE);
       Serial.print(" ,S=");Serial.print(echo.S); 
       //Serial.print(" ,SW=");
...
void Defense::lineTrace(){
    Vector v = makeV(reverseAngle(0),0);
    //キーパーダッシュ連続作動防止
     is_enough_time = (millis() - start_time > wait_time);
    dash_interval_now = millis() - start_time;
    //ラインが見えない
    if(line_angle == 400){
      state = lost_line;
    }
    ...
    //ラインもボールも認識中   通常のライントレース
       ...
       //キーパーダッシュ
       if(isNeedTo_Dash() == 1 && is_enough_time && on_dash == true){
...
uint8_t Defense::isNeedTo_Dash(){
  ...
  if(on_dash && N_ok && NE_ok && NW_ok && ball_dis > 4 && abs(ball_angle) < 40){
    return 1;
  }
  else{
    return 0;
  ...
  // if(cam_angle != 400){
  //   //機体がゴールより右側にあるとき
  //   if(cam_angle < 0 && cam_angle > -1 * echo_off_angle){
  //     NE_on = false;
  //     NW_on = true;
  //   } 
  //   //機体がゴールより左側にあるとき   
  //   else if(cam_angle > 0 && cam_angle < echo_off_angle){
  //     NE_on = true;
  //     NW_on = false;
  //   }
  //   else{
