...
const int sensorDeg[NUM_SENSORS] =  {0,15,30,45,60,75,90,105,120,135,150,165,180,195,210,225,240,255,270,285,300,315,330,345};
int sensorValue[NUM_SENSORS];
int sensorValue_setup[NUM_SENSORS];
Vector line_v;
int sensorValueMUX[4][6];// 読み取り結果を格納、[マルチプレクサ選択][チャンネル]
int sensorValueMUX_setup[4][6];
//マルチプレクサ
///////////////////////////////////////////////////////////////////////////
...
void readMUX(){
  for (int mux = 0; mux < 4; mux++) {
    ...
    for (int ch = 0; ch < 6; ch++) {//読み込む
      ...
      sensorValueMUX[mux][ch] = analogRead(analogPin);
    }  
  }
}
...
void printAllMUX(){
  for (int mux = 0; mux < 4; mux++) {
      for (int ch = 0; ch < 6; ch++) {
        Serial.print(",");
        Serial.print(sensorValueMUX[mux][ch]);
      }
    Serial.print("..");
   }
  Serial.println();
...
void calibration(){
  int sensorValueMUX_inCal[4][6][50];
  for(int count=0; count<50; count++){//50回読む
    for (int mux = 0; mux < 4; mux++) {
      for (int i = 0; i < 4; i++) { // 全MUX無効
        digitalWrite(enPins[i], HIGH);
      }
      ...
      for (int ch = 0; ch < 6; ch++) {//読み込む
        ...
        //selectChannel(ch);
        delayMicroseconds(5); // チャンネル安定待ち
        sensorValueMUX_inCal[mux][sensor[mux][ch]][count] = analogRead(analogPin);
      }  
    }
  }
    //50回読み込んだ平均を求める
    for (int mux = 0; mux < 4; mux++) {
      for (int ch = 0; ch < 6; ch++) {//読み込む
        ...
        for(int count=0; count<50;count++){
          sum = sum + sensorValueMUX_inCal[mux][ch][count];
        }
        sensorValue_setup[mux*4+ch] = sum/50;
        Serial.print(sensorValue_setup[mux*4+ch]);Serial.print(",");
      }  
    }
}
//4123865

int border = 30;//降下の度合いをしきい値に
bool isNotice(int val){
  if(sensorValue[val] - sensorValue[val] > 150){
    return 0;
  }
  else{
    return 1;
...
void toUseGame(){
    for (int mux = 0; mux < 4; mux++) {
      for (int ch = 0; ch < 6; ch++) {
         sensorValue[mux*4+ch] = sensorValueMUX[mux][ch];
      }
    }
  for (int i = 0; i < NUM_SENSORS; i++) {
    ...
    //     line_total += (1 - isNotice(sensorValue[i]));
    //   }
    //   else {
    //     line_total += isNotice(sensorValue[i]);
    //   }
    // }
    int line = 1;
...
void loop() {
  //UART();
   ...
   printAllMUX();
    for (int i = 0; i < 24; i++) {
         Serial.print(isNotice(i));
    }
  Serial.println();


    // for (int i = 0; i < 24; i++) {
    //     Serial.print(sensorValue_setup[i]);Serial.print(",");
    //   }  
    // Serial.println();
